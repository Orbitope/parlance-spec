...
cases/<case-name>/
  ...
  expected.json        what the validators must say about it (hand-written pins)
  expected.full.json   the reference validator's ENTIRE output: a sorted list of
                       [severity, code] pairs, regenerated by build_cases.py
known_divergences.json the few (case, side, issue) triples where the two
                       implementations legitimately differ, each with a reason
```

Pins only see what someone thought to pin, so both harnesses also compare the
whole multiset: the Python side asserts `expected.full.json` is current, and the
TypeScript side asserts its output equals it less the listed divergences. An
unlisted difference is a drift; a listed one that stops appearing is a stale
allowance, and both fail.
